...
function agregarResumenTarjeta() {
    ...
    $('#add-resumen-tarjeta').on('submit',function(e){
        ...
        $.ajax({
            ...
            data: formData,
            beforeSend: function(){
                $('#btn-add-res-tarj').prop('disabled',true);
            },
            success: function(resp){
                if (resp == '1') {
                    ...
                    $('#btn-add-res-tarj').prop('disabled',false);
                } else {
...
function borrarResumen() {
    ...
    $('#confirm_borrar_resumen').on('click',function(){
        $.ajax({
            ...
            beforeSend: function() {
                $('#confirm_borrar_resumen').prop('disabled',true);
            },
            success: function(resp){
                if (resp == '1') {
                    ...
                    M.toast({html: 'Ups! Ocurrió un error al agregar resumen. Intenta nuevamente', classes: 'rounded red'});
                    $('#confirm_borrar_resumen').prop('disabled',false);
                }
            },
            error: function(resp){
                if (resp.status == 500) {
...
function pagarResumen() {
    ...
    $('#confirm_pagar_resumen').on('click',function(){
        $.ajax({
            ...
            data: $('#frm-accion-resumen').serialize(),
            beforeSend: function() {                
                $('#confirm_pagar_resumen').prop('disabled',true);
            },
            success: function(resp){
                if (resp == '1') {
                    ...
                    $('#confirm_pagar_resumen').prop('disabled',false);
                }
            },
            error: function(resp){
                if (resp.status == 500) {
