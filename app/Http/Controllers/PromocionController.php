...
{
    public static function dataPromociones(){
        $data_promociones = DB::select('SELECT * FROM promociones');
        return $data_promociones;
    }
    public static function addPromocion(){
    ...
    public static function gastoPresupuesto(){
        ...
        if ( !is_null($presu) && !empty($presu) && !is_null($monto) && !empty($monto) ) {
            ...
            $monto_string = HelperController::parsearValor($monto,'$');
            $gastado_string = HelperController::parsearValor($total_gastado,'$');
