...
class ServiciosController extends Controller
{
    ...
    public static function dataFacturas() {
        ...
        foreach ($servicios as $k => $serv) {
            ...
            foreach ($data_completa as $factura) {
                ...
                // AGREGAR DATOS
                // $monto_servicio_string = explode('.',strval($factura->monto))[0];
                // if (strlen($monto_servicio_string) > 3 && strlen($monto_servicio_string) < 7) {
                //     $miles = strlen($monto_servicio_string) - 3;
                //     $monto_servicio_string = substr($monto_servicio_string, 0, $miles).'.'.substr($monto_servicio_string, -3);
                // }
                // if (strlen($monto_servicio_string) > 7 && strlen($monto_servicio_string) < 9) {
                //     $millones = strlen($monto_servicio_string) - 6;
                //     $monto_servicio_string = substr($monto_servicio_string, 0, $millones).'.'.substr($monto_servicio_string, -6, -4).'.'.substr($monto_servicio_string, -3);
                // }
                // if (isset(explode('.',strval($factura->monto))[1])) {
                //     $monto_servicio_string .= ','.explode('.',strval($factura->monto))[1];
                // }


                // $monto_mora_string = explode('.',strval($factura->monto_mora))[0];
                // if (strlen($monto_mora_string) > 3 && strlen($monto_mora_string) < 7) {
                //     $miles = strlen($monto_mora_string) - 3;
                //     $monto_mora_string = substr($monto_mora_string, 0, $miles).'.'.substr($monto_mora_string, -3);
                // }
                // if (strlen($monto_mora_string) > 7 && strlen($monto_mora_string) < 9) {
                //     $millones = strlen($monto_mora_string) - 6;
                //     $monto_mora_string = substr($monto_mora_string, 0, $millones).'.'.substr($monto_mora_string, -6, -4).'.'.substr($monto_mora_string, -3);
                // }
                // if (isset(explode('.',strval($factura->monto_mora))[1])) {
                //     $monto_mora_string .= ','.explode('.',strval($factura->monto_mora))[1];
                // }
                // DB::update("UPDATE facturas_servicio SET monto_servicio_string = '".$monto_servicio_string."', monto_mora_string = '".$monto_mora_string."' WHERE id_factura = ".$factura->id_factura);
            }
    ...
    public static function addFactura() {
        ...
        if ( !empty($servicio) ) {
            $monto_servicio_string = explode('.',strval($valor))[0];
            if (strlen($monto_servicio_string) > 3 && strlen($monto_servicio_string) < 7) {
                $miles = strlen($monto_servicio_string) - 3;
                $monto_servicio_string = substr($monto_servicio_string, 0, $miles).'.'.substr($monto_servicio_string, -3);
            }
            ...
            if (isset(explode('.',strval($valor))[1])) {
                $monto_servicio_string .= ','.explode('.',strval($valor))[1];
            }

            $monto_mora_string = explode('.',strval($valor_mora))[0];
            if (strlen($monto_mora_string) > 3 && strlen($monto_mora_string) < 7) {
                $miles = strlen($monto_mora_string) - 3;
                $monto_mora_string = substr($monto_mora_string, 0, $miles).'.'.substr($monto_mora_string, -3);
            }
            ...
            if (isset(explode('.',strval($valor_mora))[1])) {
                $monto_mora_string .= ','.explode('.',strval($valor_mora))[1];
            }
            // Insert en DDBB
            dd("INSERT into facturas_servicio (id_servicio,monto,vencimiento,segundo_vencimiento,monto_mora,monto_servicio_string,monto_mora_string) VALUES(".$servicio.",".$valor.",'".$vencimiento_1."',".$vencimiento_2.",".$valor_mora."',".$monto_servicio_string.",".$monto_mora_string.")");
